...
(add-hook 'dired-mode-hook 'enable-fixed-pitch-mode)
(add-hook 'magit-log-mode-hook 'enable-fixed-pitch-mode)
(add-hook 'shell-mode-hook 'enable-fixed-pitch-mode)
(add-hook 'term-mode-hook 'enable-fixed-pitch-mode)
#+END_SRC
...
(bind-key "C-c v" 'fixed-pitch-mode)
...
TODO: This also returns known usernames."
...
With prefix arg, prompt for USER."
  ...
  (let ((explicit-shell-file-name "dtach")
    ...
    (let* ((user-at-host (if (> (length user) 0)
                           ...
                           host))
           (default-directory (format  "/ssh:%s:" user-at-host)))
      (shell (format "*ssh-dtach %s*" user-at-host)))))
#+END_SRC
