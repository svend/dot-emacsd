...
(use-package jabber
  ...
  (progn
    ...
    (defvar hipchat-rooms '("ait" "ops" "ops-notifications" "outages")
...
;; Default value of explicit-bash-args is '("--noediting" "-i"). We
;; want login shell for remote hosts
(setq explicit-remote-bash-args
      '("--noediting" "--login"))
(setq explicit-dtach-args
      '("-A" "/tmp/ssorensen.dtach" "-z" "/bin/bash" "--noediting" "-login"))
...
(defun ssh-shell (host)
  ...
  (let ((explicit-bash-args explicit-remote-bash-args)
...
(defun ssh-dtach (host)
  ...
  (let ((explicit-shell-file-name "dtach")
	...
	(default-direct ory (format  "/ssh:%s:" host)))
    (shell (format "*ssh %s*" host))))
#+END_SRC
