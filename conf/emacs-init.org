...
Experimental function that gets environmental variables from bash. This uses
gprintenv from the homebrew coreutils package.
...
(use-package exec-path-from-shell
  ...
  (progn
    ...
    ;;    "LC_ALL"
    ;;    "SSH_AUTH_SOCK"))
    (setq exec-path-from-shell-variables (bash-shell-variables))
...
These fonts are in the [[https://packages.debian.org/sid/fonts-dejavu][fonts-dejavu]] package on Debian. On OS X, use
homebrew to install.
...
Disable visual bell until [[http://debbugs.gnu.org/cgi/bugreport.cgi?bug%3D21662][this]] is fixed.
...
(defun ssh-shell (host)
  ...
  (let ((default-directory (format  "/scpx:%s:" host)))
