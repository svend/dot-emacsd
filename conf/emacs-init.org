...
(defun bash-shell-variables()
  ...
  (let ((printenv (if (eq window-system 'ns)
                    ...
                    "printenv")))
    ;; Remove "_" to avoid warning: You appear to be setting environment variables
    ;; ("_") in your .bashrc or .zshrc
    (remove "_"
...
(use-package project
  ...
  ;; TODO: Version 0.1.2 of project module has broken git project detection. Add
  ;; the fixed version of =project-try-vc= until nixpkgs get project >= 0.1.3.
  (when (not (package-installed-p 'project '(0 1 3)))
